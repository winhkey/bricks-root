...
public class EntityUtils {
    ...
    public static Field getDeclaredField(Class<?> clazz, String fieldName) {
        return ofNullable(clazz).filter(c -> c != Object.class).map(
                apply(c -> c.getDeclaredField(fieldName), c -> getDeclaredField(c.getSuperclass(), fieldName), null, null))
                .orElse(null);
    }
    ...
    public static <T> List<T> convertMapList(List<Map<String, Object>> dataList, Class<T> clazz) {
        return isNotEmpty(dataList) ? dataList.stream().map(dataMap -> convertMap(dataMap, clazz)).collect(toList())
                : null;
    }
    ...
    public static Map<String, Object> convertData(Object object, String... excludes) {
        return ofNullable(object).map(o -> {
            ...
            list.stream().filter(field -> isEmpty(excludes) || !contains(excludes, field.getName()))
    ...
    public static Object getEnumValue(Class<?> clazz, Object value) {
        Stream<?> stream = ValueEnum.class.isAssignableFrom(clazz)
                ? of(((Class<ValueEnum<?>>) clazz).getEnumConstants()).filter(v -> v.getValue().equals(value))
                : of(clazz.getEnumConstants()).filter(v -> v.equals(value));
    ...
    public static Class<?> getComponentClass(Class<?> selfClass, Class<?> superClass, int index) {
        return getResolvableType(selfClass, superClass).getGeneric(index).resolve();
    }
    ...
    public ResolvableType getResolvableType(Class<?> selfClass, Class<?> superClass) {
        return ResolvableType.forClass(selfClass).as(superClass);
    }
